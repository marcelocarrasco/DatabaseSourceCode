--------------------------------------------------------
--  DDL for Procedure IMPORT_LTE_MOBILITY_MANAGEMENT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."IMPORT_LTE_MOBILITY_MANAGEMENT" (
             ...
             l_errors := sql%bulk_exceptions.count;
                ...
                loop
                    ...
                    INSERT INTO ERROR_LOG(ID, IMPORT_DATE, SOURCE, TARGET,FINS_ID, PERIOD_START_TIME,
                     MCC_ID, MNC_ID,TA_ID, ERROR_NRO, ERROR_MESG)
                    VALUES
                    (seq_error_log.NEXTVAL,sysdate,p_source1_table,p_target_table,
                     t_array_col(l_idx).FINS_ID,t_array_col(l_idx).PERIOD_START_TIME,t_array_col(l_idx).MCC_ID,
                       t_array_col(l_idx).MNC_ID,t_array_col(l_idx).TA_ID, l_errno,l_msg);
