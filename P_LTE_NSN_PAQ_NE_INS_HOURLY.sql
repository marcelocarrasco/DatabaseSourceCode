--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_PAQ_NE_INS_HOURLY
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_PAQ_NE_INS_HOURLY" (P_FECHA_DESDE     IN CHAR,
...
SELECT DECODE(P_LEVEL,
                      'HOUR', 'PowerAndQualityNeHour') TABLA_A_TRUNCAR,
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA, V_CANTIDAD_SEMANAS) LOOP
    ...
    IF P_LEVEL = 'HOUR' THEN
      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          P_LTE_NSN_PAQ_LNB_HOUR_INS (SEN.FECHA, SEN.FECHA);
      ELSE
          P_LTE_NSN_PAQ_NE_HOUR_INS (P_SUBCLASS_TABLE, SEN.FECHA, SEN.FECHA);
      END IF;
