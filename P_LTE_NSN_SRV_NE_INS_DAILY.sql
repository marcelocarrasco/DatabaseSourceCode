--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_SRV_NE_INS_DAILY
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_SRV_NE_INS_DAILY" (P_FECHA_DESDE     IN CHAR,
...
SELECT DECODE(P_LEVEL, 'BH', 'ServiceNeBH',
                      'DAY', 'ServiceNeDay') TABLA_A_TRUNCAR,
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA, V_CANTIDAD_SEMANAS) LOOP
    ...
    IF P_LEVEL = 'DAY' THEN
      ...
      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          P_LTE_NSN_SRV_LNB_DAY_INS (SEN.FECHA, SEN.FECHA);
      ELSE
          P_LTE_NSN_SRV_NE_DAY_INS (P_SUBCLASS_TABLE, SEN.FECHA, SEN.FECHA);
      END IF;

    ELSIF P_LEVEL = 'BH' THEN

      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          P_LTE_NSN_SRV_LNB_BH_INS (SEN.FECHA, SEN.FECHA);
      ELSE
          P_LTE_NSN_SRV_NE_BH_INS (P_SUBCLASS_TABLE, SEN.FECHA, SEN.FECHA);
      END IF;
