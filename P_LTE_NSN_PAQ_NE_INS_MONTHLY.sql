--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_PAQ_NE_INS_MONTHLY
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_PAQ_NE_INS_MONTHLY" (P_FECHA_DESDE     IN CHAR,
...
SELECT DECODE(P_LEVEL, 'IBHM' , 'PaQNeIBHM',
                      'DAYM'   , 'PaQNeDayM' ) TABLA_A_TRUNCAR,
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA, V_CANTIDAD_MESES) LOOP
    ...
    IF P_LEVEL = 'DAYM' THEN
      ...
      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          ...
          P_LTE_NSN_PAQ_LNB_DAYM_INS (SEN.FECHA_CHR);
      ELSE
          --DBMS_OUTPUT.PUT_LINE('TEST');
          P_LTE_NSN_PAQ_NE_DAYM_INS (P_SUBCLASS_TABLE, SEN.FECHA_CHR);
      END IF;
    ...
    ELSIF P_LEVEL = 'IBHM' THEN
      ...
      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          --DBMS_OUTPUT.PUT_LINE('TEST');
          P_LTE_NSN_PAQ_LNB_IBHM_INS (SEN.FECHA_CHR);
      ELSE
          --DBMS_OUTPUT.PUT_LINE('TEST');
          P_LTE_NSN_PAQ_NE_IBHM_INS (P_SUBCLASS_TABLE, SEN.FECHA_CHR);
      END IF;
