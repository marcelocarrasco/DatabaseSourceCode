--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_AVA_LCE_INS_HOURLY_R
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_AVA_LCE_INS_HOURLY_R" (P_FECHA_DESDE     IN CHAR,
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA) LOOP
   ...
   P_LTE_TRUNCATE_SPART_HOURLY (SEN.FECHA_DESDE, SEN.FECHA_HASTA, V_TABLA_A_TRUNCAR, P_SUBCLASS_TABLE);
    ...
    IF P_LEVEL = 'HOUR' THEN
      ...
      P_LTE_NSN_AVA_LCE_HOUR_INS (SEN.FECHA_DESDE, SEN.FECHA_HASTA);
