--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_AVA_NE_INS_HOURLY
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_AVA_NE_INS_HOURLY" (P_FECHA_DESDE     IN CHAR,
                                                                    P_FECHA_HASTA     IN CHAR,
                                                                    P_SUBCLASS_TABLE  IN CHAR,
                                                                    P_LEVEL           IN CHAR) IS

-- Autor: Mariano Morón. Fecha: 11.09.2015.
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA, V_CANTIDAD_SEMANAS) LOOP
    ...
    IF P_LEVEL = 'HOUR' THEN
      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          P_LTE_NSN_AVA_LNB_HOUR_INS (SEN.FECHA, SEN.FECHA);
      ELSE
          P_LTE_NSN_AVA_NE_HOUR_INS (P_SUBCLASS_TABLE, SEN.FECHA, SEN.FECHA);
      END IF;
