--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_SRV_NE_INS_WEEKLY
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_SRV_NE_INS_WEEKLY" (P_FECHA_DESDE     IN CHAR,
...
SELECT DECODE(P_LEVEL, 'IBHW', 'ServiceNeIBHW',
                       'DAYW'   , 'ServiceNeDayW'    ) TABLA_A_TRUNCAR,
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA, V_CANTIDAD_SEMANAS) LOOP
    ...
    IF P_LEVEL = 'DAYW' THEN
      ...
      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          P_LTE_NSN_SRV_LNB_DAYW_INS (SEN.FECHA_CHR);
      ELSE
          P_LTE_NSN_SRV_NE_DAYW_INS (P_SUBCLASS_TABLE, SEN.FECHA_CHR);
      END IF;

    ELSIF P_LEVEL = 'IBHW' THEN

      IF P_SUBCLASS_TABLE = 'LNBTS' THEN
          P_LTE_NSN_SRV_LNB_IBHW_INS (SEN.FECHA_CHR);
      ELSE
          P_LTE_NSN_SRV_NE_IBHW_INS (P_SUBCLASS_TABLE, SEN.FECHA_CHR);
      END IF;
