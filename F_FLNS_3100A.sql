--------------------------------------------------------
--  DDL for Function F_FLNS_3100A
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "HARRIAGUE"."F_FLNS_3100A" (P_FINI varchar2,P_FFIN varchar2 default null) return FLNS_TAB PIPELINED as
begin
 if p_ffin is null then
    for I in (select  FLNS.PERIOD_START_TIME FECHA, 
              ...
              INNER JOIN (
    --                  select SGSN_NAME as MME_NAME,SGSN_ID 
    --                  FROM VW_SGSN_OBJECTS
      ...
      pipe row (FLNS_OBJ(i.FECHA,i.PERIOD_DURATION,i.FINS_ID,i.MME_NAME,i.TA_ID,i.FLNS_3100A));
    end LOOP;
  else
    for I in (select  FLNS.PERIOD_START_TIME FECHA, 
                      FLNS.PERIOD_DURATION PERIOD_DURATION,
