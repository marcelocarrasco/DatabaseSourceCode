--------------------------------------------------------
--  DDL for View FLNS_3109B_CONS_ISABHMONTH
--------------------------------------------------------

  CREATE OR REPLACE VIEW "HARRIAGUE"."FLNS_3109B_CONS_ISABHMONTH" ("FECHA", "PERIOD_DURATION", "FINS_ID", "MME_NAME", "FLNS_3109B") AS 
  SELECT TRUNC(hist.FECHA , 'MONTH') AS FECHA, hist.PERIOD_DURATION, hist.FINS_ID, hist.MME_NAME,
   ROUND(AVG(hist.FLNS_3109B),2 ) AS FLNS_3109B
FROM(
...
INNER JOIN (
           ...
           FROM (
                  ...
                  FROM PCOFNS_PS_SGSM_FNS_CONS_BH_AUX
                  )
           WHERE SEQNUM < 4
        ) bh
