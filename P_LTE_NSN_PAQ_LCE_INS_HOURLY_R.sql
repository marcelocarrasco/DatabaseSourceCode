--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_PAQ_LCE_INS_HOURLY_R
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_PAQ_LCE_INS_HOURLY_R" (P_FECHA_DESDE     IN CHAR,
...
-- Autor: Mariano Morón. Fecha: 19.10.2015.
...
SELECT DECODE(P_LEVEL, 'HOUR', 'PowerAndQualityLceHour') TABLA_A_TRUNCAR
...
IF V_TABLA_A_TRUNCAR IS NOT NULL THEN
  ...
  P_LTE_TRUNCATE_SPART_HOURLY (SEN.FECHA_DESDE, SEN.FECHA_HASTA, V_TABLA_A_TRUNCAR, P_SUBCLASS_TABLE);
    ...
    IF P_LEVEL = 'HOUR' THEN
      ...
      P_LTE_NSN_PAQ_LCEL_HOUR_INS (SEN.FECHA_DESDE, SEN.FECHA_HASTA);
