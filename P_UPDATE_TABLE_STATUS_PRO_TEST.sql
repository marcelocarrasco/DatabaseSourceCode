--------------------------------------------------------
--  DDL for Procedure P_UPDATE_TABLE_STATUS_PRO_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_UPDATE_TABLE_STATUS_PRO_TEST" (
p_external_import_table     IN VARCHAR)
 IS
  ...
  TYPE t_array_tab IS TABLE OF STATUS_PROCESS_ETL_TEST%ROWTYPE;
  ...
  LOOP
    ...
    BEGIN
      FORALL i IN 1 .. t_array_col.COUNT SAVE EXCEPTIONS
        INSERT INTO STATUS_PROCESS_ETL_TEST VALUES t_array_col (i);
   EXCEPTION
        ...
        -- Capture exceptions to perform operations DML
             l_errors := sql%bulk_exceptions.count;
                ...
                loop
                    ...
                    INSERT INTO ERROR_LOG_TEST(ID, FILENAME, ERROR_NRO, ERROR_MESG)
                    VALUES
                    (seq_error_log.NEXTVAL,t_array_col(l_idx).FILENAME, l_errno,l_msg );
