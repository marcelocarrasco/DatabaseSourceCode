--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_AVA_LCE_INS_WEEKLY_R
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_AVA_LCE_INS_WEEKLY_R" (P_FECHA_DESDE     IN CHAR,
                                                             P_FECHA_HASTA      IN CHAR,
                                                             P_SUBCLASS_TABLE   IN CHAR,
                                                             P_LEVEL            IN CHAR,
                                                            P_RESULT OUT NUMBER) IS

-- Autor: Mariano Morón. Fecha: 13.10.2015.
...
SELECT DECODE(P_LEVEL, 'IBHW' , 'AvailabilityWcellIBHW',
                       'DAYW'   , 'AvailabilityWcellDayW'    ) TABLA_A_TRUNCAR
...
IF V_TABLA_A_TRUNCAR IS NOT NULL THEN
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA) LOOP
    ...
    P_LTE_NSN_AVA_LCE_DAYW_INS (SEN.FECHA_CHR);

    ELSIF P_LEVEL = 'IBHW' THEN

      P_LTE_TRAFIC_LCEL_IBHW_AUX(SEN.FECHA_CHR);

      P_LTE_NSN_AVA_LCEL_IBHW_INS (SEN.FECHA_CHR);
