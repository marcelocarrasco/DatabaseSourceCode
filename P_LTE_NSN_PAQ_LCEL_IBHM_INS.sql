--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_PAQ_LCEL_IBHM_INS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_PAQ_LCEL_IBHM_INS" (P_FECHA_DESDE IN CHAR)
IS
...
TYPE T_ARRAY_TAB IS TABLE OF LTE_NSN_PAQ_LCEL_IBHM%ROWTYPE;
...
CURSOR CUR (FECHA_DESDE VARCHAR2, FECHA_HASTA VARCHAR2) IS
  SELECT
    ...
    T1.LNBTS_ID,
    T1.LNCEL_ID,
    NVL(ROUND(AVG(T1.RSSI_PUSCH_UL_NUM),2)                            ,0 )           AS   RSSI_PUSCH_UL_NUM,
    NVL(ROUND(AVG(T1.RSSI_PUSCH_UL_DEN),2)                            ,0 )           AS   RSSI_PUSCH_UL_DEN,
    NVL(ROUND(AVG(T1.RSSI_PUCCH_UL_NUM),2)                            ,0 )           AS   RSSI_PUCCH_UL_NUM,
    NVL(ROUND(AVG(T1.RSSI_PUCCH_UL_DEN),2)                            ,0 )           AS   RSSI_PUCCH_UL_DEN,
    NVL(ROUND(AVG(T1.SINR_PUSCH_UL_NUM),2)                            ,0 )           AS   SINR_PUSCH_UL_NUM,
    NVL(ROUND(AVG(T1.SINR_PUSCH_UL_DEN),2)                            ,0 )           AS   SINR_PUSCH_UL_DEN,
    NVL(ROUND(AVG(T1.SINR_PUCCH_UL_NUM),2)                            ,0 )           AS   SINR_PUCCH_UL_NUM,
    NVL(ROUND(AVG(T1.SINR_PUCCH_UL_DEN),2)                            ,0 )           AS   SINR_PUCCH_UL_DEN,
    NVL(ROUND(AVG(T1.CQI_NUM),2)                                      ,0 )           AS   CQI_NUM,
    NVL(ROUND(AVG(T1.CQI_DEN),2)                                      ,0 )           AS   CQI_DEN
  FROM LTE_NSN_PAQ_LCEL_BH T1
...
LOOP
  ...
  BEGIN
    FORALL I IN 1 .. T_ARRAY_COL.COUNT SAVE EXCEPTIONS
      INSERT INTO LTE_NSN_PAQ_LCEL_IBHM VALUES T_ARRAY_COL (I);
  EXCEPTION
