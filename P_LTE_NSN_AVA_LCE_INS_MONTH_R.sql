--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_AVA_LCE_INS_MONTH_R
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_AVA_LCE_INS_MONTH_R" (P_FECHA_DESDE          IN CHAR,
...
SELECT DECODE(P_LEVEL, 'IBHM' , 'AvailabilityWcellIBHM',
                      'DAYM'   , 'AvailabilityWcellDayM'    ) TABLA_A_TRUNCAR
...
IF V_TABLA_A_TRUNCAR IS NOT NULL THEN
  ...
  FOR SEN IN PERIODO (P_FECHA_DESDE, P_FECHA_HASTA) LOOP
    ...
    IF P_LEVEL = 'DAYM' THEN
      ...
      P_LTE_NSN_AVA_LCE_DAYM_INS (SEN.FECHA_CHR);

    ELSIF P_LEVEL = 'IBHM' THEN

      P_LTE_TRAFIC_LCEL_IBHM_AUX(SEN.FECHA_CHR);

      P_LTE_NSN_AVA_LCEL_IBHM_INS (SEN.FECHA_CHR);
