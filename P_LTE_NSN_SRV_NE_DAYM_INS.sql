--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_SRV_NE_DAYM_INS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_SRV_NE_DAYM_INS" (P_ELE_CLASS IN CHAR, P_FECHA_DESDE IN CHAR)
IS
...
TYPE T_ARRAY_TAB IS TABLE OF LTE_NSN_SERVICE_NE_DAYM%ROWTYPE;
...
CURSOR CUR (ELE_CLASS VARCHAR2, FECHA_DESDE VARCHAR2, FECHA_HASTA VARCHAR2) IS
  ...
  FROM LTE_NSN_SERVICE_NE_DAYM T1
  ...
  LOOP
    ...
    BEGIN
      FORALL I IN 1 .. T_ARRAY_COL.COUNT SAVE EXCEPTIONS
        INSERT INTO LTE_NSN_SERVICE_NE_DAYM VALUES T_ARRAY_COL (I);
    EXCEPTION
