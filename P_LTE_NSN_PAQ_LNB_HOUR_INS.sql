--------------------------------------------------------
--  DDL for Procedure P_LTE_NSN_PAQ_LNB_HOUR_INS
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "HARRIAGUE"."P_LTE_NSN_PAQ_LNB_HOUR_INS" (P_FECHA_DESDE IN CHAR, P_FECHA_HASTA IN CHAR)
IS
...
TYPE T_ARRAY_TAB IS TABLE OF LTE_NSN_PAQ_NE_HOUR%ROWTYPE;
 ...
 BEGIN
  ...
  SELECT
    ...
    FROM LTE_NSN_PAQ_LCEL_HOUR T1
        LEFT OUTER JOIN LTE_NSN_PAQ_NE_HOUR T2
    ON TO_CHAR(T1.LNBTS_ID) = T2.ELEMENT_ID
        AND T1.FECHA= T2.FECHA
    WHERE T1.FECHA BETWEEN TO_DATE(P_FECHA_DESDE, 'DD.MM.YYYY HH24')
  ...
  LOOP
    ...
    BEGIN
      FORALL I IN 1 .. T_ARRAY_COL.COUNT SAVE EXCEPTIONS
        INSERT INTO LTE_NSN_PAQ_NE_HOUR VALUES T_ARRAY_COL (I);
    EXCEPTION
