--------------------------------------------------------
--  DDL for View FLNS_5029B_HISTORICAL
--------------------------------------------------------

  CREATE OR REPLACE VIEW "HARRIAGUE"."FLNS_5029B_HISTORICAL" ("FECHA", "PERIOD_DURATION", "FINS_ID", "MME_NAME", "EPS_ECM_CONN_SUM", "EPS_ECM_CONN_DENOM") AS 
  SELECT FLNS.PERIOD_START_TIME AS FECHA,
       FLNS.PERIOD_DURATION,
       FLNS.FINS_ID,
       MO.MME_NAME,
       FLNS.EPS_ECM_CONN_SUM,
       FLNS.EPS_ECM_CONN_DENOM
  FROM (SELECT PERIOD_START_TIME,
               PERIOD_DURATION,
               FINS_ID,
               SUM(EPS_ECM_CONN_SUM) AS EPS_ECM_CONN_SUM,
               AVG(EPS_ECM_CONN_DENOM) AS EPS_ECM_CONN_DENOM
